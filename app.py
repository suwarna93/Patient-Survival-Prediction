...
st.set_page_config(page_title='Patient Survival Prediction', page_icon="🏥", layout="wide", initial_sidebar_state='expanded')

model = load_model('keras_model.h5')

# creating option list for dropdown menu

features = ['gcs_motor_apache', 'd1_lactate_max', 'd1_lactate_min','apache_4a_hospital_death_prob', 'apache_4a_icu_death_prob']
...
def main():
    with st.form('prediction_form'):
        ...
        gcs_motor_apache = st.slider('gcs_motor_apache', 1.0000, 7.0000, value=1.0000, format="%f")
    ...
    if submit:
        ...
        data= np.array([gcs_motor_apache, d1_lactate_max, d1_lactate_min, apache_4a_hospital_death_prob, apache_4a_icu_death_prob]).reshape(1, -1)
